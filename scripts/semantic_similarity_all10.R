...
process_model <- function(filepath, model_name) {
  ...
  for (i in 1:nrow(data)) {
    ...
    if (i %% 500 == 0) cat(sprintf("  Processed %d/%d rows\n", i, nrow(data)))
  }
  return(results)
}
...
all_results <- rbind(
  ...
  process_model(paste0(base_path, "Qwen_Qwen3-32B.jsonl"), "Qwen3 32B"),
  process_model(paste0(base_path, "google_gemma-4-31b.jsonl"), "Gemma-4 31B"),
  process_model(paste0(base_path, "llama3.1-8b.jsonl"), "Llama 3.1 8B"),
  process_model(paste0(base_path, "mixtral-8x7b.jsonl"), "Mixtral 8x7B"),
  process_model(paste0(base_path, "qwen2.5-7b.jsonl"), "Qwen2.5 7B"),
  process_model(paste0(base_path, "mistral-7b.jsonl"), "Mistral 7B"),
  process_model(paste0(base_path, "gemma3-12b.jsonl"), "Gemma3 12B")
)

write.csv(all_results,
          "/Users/kallolnaha/Documents/mix_projects/mindrouter/PROBE/semantic_similarity_results_all10.csv",
          row.names = FALSE)
cat("Results saved!\n")

summary_table <- aggregate(llm_sim ~ model + namespace,
                           data = all_results,
